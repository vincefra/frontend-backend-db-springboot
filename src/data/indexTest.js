...
const maxAnnularSectors = 2; // total annular sectors = maxAnnular + 1 ('other')
...
async function getData() {
  function getTechList(technologies, clientId, projectId, employeeId) {
    ...
    return (techList = techList.map(technology => {
      let techObj = technologyList.find(
        t => t.name.toLowerCase() === technology.toLowerCase()
      );
      if (techObj) {
  ...
  function getClientColor(client) {
    if (!client) return -1;
    const clientObj = clientList.find(
      c => c.name.toLowerCase() === client.toLowerCase()
    );
  ...
  function updateClient(clientId, projectId, duration, employees, skills) {
    if (!clientList[clientId].projects.includes(projectId))
      clientList[clientId].projects.push(projectId);
  ...
  function getEmployeeList(employees) {
    ...
    return empList.map(
      employee =>
        employeeList.find(e => e.name.toLowerCase() === employee.toLowerCase())
          .id
    );
  }
  ...
  async function fetchData() {
    ...
    // { "id": 2, "firstName":"Peter" , "lastName":"Jones" ,"role": "test", "birthYear": 2019, "startYear": 2019, "endYear": 2019, "location": "Kista", "technologies": "javascript", "languages": "Java"} ]`;
    let client = `[
    { "id": 0, "name":"Håkan" , "category":"Transport"  ,"location": "Kista", "description": "javascript"},
    { "id": 1, "name":"Tommy" , "category":"Transport" ,"location": "Kista", "description": "javascript"},
    { "id": 2, "name":"Peter" , "category":"Transport" ,"location": "Kista", "description": "javascript"} ]
    `;
    let project = `[
    { "id": 0, "name":"Håkan" , "type":"Schön"  , "startDates": "2019-10-01", "endDates": "2019-10-01", "client": "Håkan", "employees": "Håkan Schön", "description": "javascript", "technologies": "Java"},
    { "id": 1, "name":"Tommy" , "type":"Fagerberg" , "startDates": "2019-10-01", "endDates": "2019-10-01", "client": "Tommy", "employees": "Tommy Fagerberg", "description": "javascript", "technologies": "Java"},
    { "id": 2, "name":"Peter" , "type":"Jones" , "startDates": "2019-10-01", "endDates": "2019-10-01", "client": "Peter", "employees": "Peter Jones", "description": "javascript", "technologies": "Java"} ]`;
    const requestURL = "http://localhost:7878/api/findall";
    const options = {
      url: requestURL,
      method: "GET",
      headers: {}
  ...
  // fetch("http://localhost:7878/api/findall")
  // .then(response => {
  ...
  for (let employee of employees) {
    let img;
    try {
      img = `img/employees/${[employee.firstName, employee.lastName]
        .join("_")
        .replace(/\s/g, "_")}.jpg`;
    ...
    employeeList.push({
      ...
      dateEnd: `${
        employee.endYear
          ? moment(employee.endYear + "-01-01").format(dateFormat)
...
async function groupCategories(clients) {
  ...
  categories[maxAnnularSectors].hours =
    categories[maxAnnularSectors].list.length;
...
export function getLargestClients(clients) {
  ...
  for (let i = maxAnnularSectors; i < clients.length; i++) {
    ...
    else other.clients.push(clients[i].id);
